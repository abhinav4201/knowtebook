...
?>
<?php
    $result= queryMysql("SELECT * FROM `requirement` WHERE username='$user' ORDER BY `issue no` DESC");
    if($result->num_rows)
    {
                    ...
                    <?php
                        if($result->num_rows)
                        {
                            for ($j = 0 ; $j < $rows ; ++$j)
                            {
                                ...
                                echo '<div class="in500"><b>Issue No.'. $row['issue no'].'</b>'.'&nbsp;&nbsp;'.'<span class="available">';
                                echo timeAgo(strtotime(date(("Y-m-d"),$pt))).'</span></div>';
                                echo '<div class="it500"><a href="yourrequirecmnt.php?content='.$it.'">'. ucfirst($row['issue']).'</a></div>';
