    ...
    <div data-role="page" id="" data-url="">
        ...
        <main>      
            <div data-role="main" id="main01" class="ui-content jqm-fullwidth jqm-content">
                ...
                <div id="fil500">
                    <div id="fil501">Filter</div>
                    <select name="category" id="fil502" class="input" onChange="changeCat(this)">
                    ...
                    <?php
                        if($result->num_rows)
                        {
                            for ($j = 0 ; $j < $rows ; ++$j)
                            {
                                ...
                                $it= $row['issue no'];
                                $usr=$row['username'];
                                echo '<div class="in500" aria-label="issue"><b>Issue No.'. $row['issue no'].'</b>'.'&nbsp;&nbsp;&#8652;&nbsp;&nbsp;'.'<a href="member.php?content='.$usr.'">'.$row['username'].'</a>&nbsp;&nbsp;&#9872;&nbsp;&nbsp;'.'<span class="available">';
                                echo timeAgo(strtotime($row['posting time'])).'</span></div>';
                                echo '<div class="it500" aria-label="title"><a href="yourrequirecmnt.php?content='.$it.'">'. ucfirst($row['issue']).'</a></div>';
                                echo '<div class="des500" aria-label="description">Description: &nbsp;'.ucfirst($row['description']).'</div>';                            
                            }
                        }
                    ?>
                </div>
