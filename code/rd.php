...
?>
<?php
    $result= queryMysql("SELECT * FROM `research paper` ORDER BY `no` DESC");
    if($result->num_rows)
    {
                    ...
                    <?php
                        if($result->num_rows)
                        {
                            for ($j = 0 ; $j < $rows ; ++$j)
                            {
                                ...
                                echo '<div class="in500"><b>RESEARCH PAPER.'. $row['no'].'</b>'.'&nbsp;&nbsp;&#8652;&nbsp;&nbsp;'.'<a href="member.php?content='.$usr.'">'.$row['username'].'</a>&nbsp;&nbsp;&#9872;&nbsp;&nbsp;'.'<span class="available">';
                                echo date(("Y-m-d"),$pt).'</span></div>';
                                echo '<div class="it500"><a href="yourrdcmnt.php?content='.$it.'">'. ucfirst($row['title']).'</a></div>';
                                echo '<div class="des500">INTRODUCTION: &nbsp;'.ucfirst($row['introduction']).'</div>';                            
                            }
                        }
                    ?>
                </div>
